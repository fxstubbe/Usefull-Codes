...
fastq_name = scan("name_fastq_170601_v2.txt",what=character(0),nlines=1,sep="\t")
...
nSeq = length(newseqlabels)
for(i in 1:nSeq) {
  cat(paste("perl vcf-query ~/Richard_copin/Projects/Shopsin/Deep_sequencing/vcf_files/",ReFF_namelabels[9],"/", newseqlabels[i],'.raw.snps.indels.vcf -f "%CHROM\\t%POS\\t%REF\\t%ALT\\t%QUAL\\t%INFO/DP\\t%INFO/AF\\n" > ~/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/annovar/',newseqlabels[i], '.txt', sep=""),sep="\n",file=VCF, append = TRUE)
  cat("Output",i,"of",nSeq,"\n")
}


cat("cp ~/Richard_copin/Projects/Shopsin/Deep_sequencing/vcf_files/USA300_FPR3757/VCFtool.sh /Users/copinr01/Richard_copin/Ressources/Programs/vcftools/src/perl/.")
cat("cd Richard_copin/Ressources/Programs/vcftools/src/perl/")
cat("chmod +x /Users/copinr01/Richard_copin/Ressources/Programs/vcftools/src/perl/VCFtool.sh")


#check depth of coverage
setwd("~/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/annovar/")
for (i in 1:nSeq) {
...
for(i in 1:nSeq) {
  cat(paste("perl annotate_variation.pl ", newseqlabels.vcf[i]," --buildver ", ReFF_namelabels[9]," ", ReFF_namelabels[9],"/", sep=""),sep="\n",file=Anno, append = TRUE)
  cat("Output",i,"of",nSeq,"\n")
}
cat("chmod +x /Users/copinr01/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/annovar/Annovar.sh")
cat(paste("mv *exonic* ../", ReFF_namelabels[9],"/exonic/. | mv *inval* ../", ReFF_namelabels[9],"/invalid/. | mv *variant_f* ../", ReFF_namelabels[9],"/intergenic/. | rm *.log ", sep = ""))


# Invalid output
setwd(paste("~/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/",ReFF_namelabels[9],"/invalid/", sep = ""))
for(i in 1:nSeq) {
...
paste("mv ../", ReFF_namelabels[9],"/invalid/*invalid.txt .", sep = "")
...
for(i in 1:nSeq) {
  cat(paste("perl annotate_variation.pl ", newseqlabels.invalidoutput[i]," --buildver ", ReFF_namelabels[9]," ", ReFF_namelabels[9],"/", sep=""),sep="\n",file=AnnoInvl, append = TRUE)
  cat("Output",i,"of",nSeq,"\n")
}
cat("chmod +x /Users/copinr01/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/annovar/Annovar_invalid.sh")
cat(paste("mv *exonic* ../", ReFF_namelabels[9],"/exonic/. | mv *variant_f* ../", ReFF_namelabels[9],"/intergenic/. | rm *.log | rm *_input", sep = ""))
...
setwd(paste("~/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/",ReFF_namelabels[9],"/exonic/", sep = ""))
...
setwd(paste("~/Richard_copin/Projects/Shopsin/Deep_sequencing/annotation/",ReFF_namelabels[9],"/intergenic/", sep = ""))
